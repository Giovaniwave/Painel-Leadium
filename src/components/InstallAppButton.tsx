...
export default function InstallAppButton() {
  ...
  const modalContent = showInstructions && (
    <div 
      className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md" 
      onClick={() => setShowInstructions(false)}
    >
      <div 
        className="bg-[#141414] border border-white/10 rounded-2xl w-full max-w-sm p-6 shadow-2xl relative my-auto animate-fadeIn mx-auto"
  ...
  return (
    <>
      ...
      {/* Render modal directly in body to avoid portal parent transform limitations */}
      {showInstructions && typeof document !== 'undefined' && createPortal(modalContent, document.body)}
    </>
  );
}
